    ...
    async fn run_monitor_async(
        ...
        loop {
            ...
            match connect_async(url).await {
                Ok((mut ws_stream, _)) => {
                    crate::append_log(&app, format!("[{}] Successfully connected to X-Plane 12 WebSocket.", Local::now().format("%Y-%m-%d %H:%M:%S")));
                    {
                    ...
                    while let Some(Ok(msg)) = ws_stream.next().await {
                        ...
                        if let Message::Text(text) = msg {
                            if let Ok(data) = serde_json::from_str::<serde_json::Value>(&text) {
                                ...
                                if let Some(lat) = data["sim/flightmodel/position/latitude"].as_f64() { m.latitude = lat; }
                                if let Some(lon) = data["sim/flightmodel/position/longitude"].as_f64() { m.longitude = lon; }
                                if let Some(alt) = data["sim/flightmodel/position/elevation"].as_f64() { m.gps_altitude_msl = alt * 3.28084; m.indicated_altitude = alt * 3.28084; }
                                if let Some(roll) = data["sim/flightmodel/position/phi"].as_f64() { m.roll_angle = roll; }
                                if let Some(pitch) = data["sim/flightmodel/position/theta"].as_f64() { m.pitch_angle = pitch; }
                                if let Some(hdg) = data["sim/flightmodel/position/psi"].as_f64() { m.heading = hdg; m.track = hdg; }
                                if let Some(ias) = data["sim/flightmodel/position/indicated_airspeed"].as_f64() { m.indicated_airspeed = ias; }
                                if let Some(gs) = data["sim/flightmodel/position/groundspeed"].as_f64() { m.ground_speed = gs * 1.94384; }
                                if let Some(vs) = data["sim/flightmodel/position/vh_ind"].as_f64() { m.vertical_speed = vs * 196.85; }
                                if let Some(onground) = data["sim/flightmodel/failures/onground_any"].as_i64() { m.is_on_ground = onground as f64; }
                                
                                // X-Plane specific
                                if let Some(rpm) = data["sim/flightmodel/engine/ENGN_RPM"].get(0).and_then(|v| v.as_f64()) { m.xp_prop_rpm = rpm; }
                                ...
                                if ongoing && !flight_ongoing {
                                    flight_ongoing = true;
                                    crate::append_log(&app, format!("[{}] [X-Plane] Detected ongoing flight. Starting log.", Local::now().format("%H:%M:%S")));
                                    
                                    // Initialize DB
                                    let app_data_dir = app.path().app_data_dir().unwrap();
                                    let internal_log_dir = app_data_dir.join("flightlogs");
                                    let _ = create_dir_all(&internal_log_dir);
                                    let filename = format!("butterlog_xp_{}.db", Local::now().format("%Y%m%d_%H%M%S"));
                                    ...
                                    if m.is_on_ground < 0.5 {
                                        if let Some(db) = app.try_state::<AirportsDatabase>() {
                                            if let Some(nearest) = db.find_nearest(m.latitude, m.longitude, 1).first() {
                                                let dist = calculate_distance(m.latitude, m.longitude, nearest.latitude_deg.unwrap_or(0.0), nearest.longitude_deg.unwrap_or(0.0));
    ...
    fn start(&self, app: AppHandle, log_path: Option<PathBuf>) -> anyhow::Result<()> {
        ...
        *running = true;
        ...
        thread::spawn(move || {
            ...
            rt.block_on(async {
                let _ = Self::run_monitor_async(app, metrics, running_clone, connected_clone, log_path).await;
            });
        });
